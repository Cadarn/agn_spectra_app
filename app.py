...
# Controllers
inclination = st.sidebar.slider("Inclination angle",
                                min_value=0.,
                                max_value=90.,
                                step=0.1,
...
opening_angle = st.sidebar.slider("Torus opening angle",
                                  min_value=0.,
                                  max_value=90.,
                                  step=0.1,
                                  format=f'%.1f{degree_symbol}',
...
column_density = st.sidebar.slider("Torus column density, logNH",
                                   min_value=22.,
                                   max_value=25.5,
                                   step=0.1,
                                   format=f'%.1f',
...
mod_df = df.melt(id_vars=['Energy'], value_vars=['Transmitted', "Reprocessed", "Summed"])
mod_df = mod_df.rename(columns={"variable": "Model Component", "value": "Flux"})

# Construct our plot
fig = px.line(mod_df, x="Energy", y="Flux", color='Model Component',
              log_x=True, width=900, height=500,
